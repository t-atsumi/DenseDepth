...
def get_eyemodel_data(batch_size, num_frames, eyemodel_data_zipfile='eyemodel_data.zip'):
    ...
    shape_rgb = (num_frames, batch_size, 320, 320, 3)
    shape_depth = (num_frames, batch_size, 160, 160, 1)

    return data, eyemodel_train, eyemodel_test, shape_rgb, shape_depth

def get_eyemodel_train_test_data(batch_size, num_frames):
    data, eyemodel_train, eyemodel_test, shape_rgb, shape_depth = get_eyemodel_data(batch_size=batch_size, num_frames=num_frames)

    train_generator = EyeModel_BasicAugmentRGBSequence(data, eyemodel_train, batch_size=batch_size, num_frames=num_frames, shape_rgb=shape_rgb, shape_depth=shape_depth)
    test_generator = EyeModel_BasicRGBSequence(data, eyemodel_test, batch_size=batch_size, num_frames=num_frames, shape_rgb=shape_rgb, shape_depth=shape_depth)

    return train_generator, test_generator

class EyeModel_BasicAugmentRGBSequence(Sequence):
    def __init__(self, data, dataset, batch_size, num_frames, shape_rgb, shape_depth, is_flip=False, is_addnoise=False, is_erase=False):
        self.data = data
        self.dataset = dataset
        self.policy = BasicPolicy( color_change_ratio=0.50, mirror_ratio=0.50, flip_ratio=0.0 if not is_flip else 0.2, 
                                    add_noise_peak=0 if not is_addnoise else 20, erase_ratio=-1.0 if not is_erase else 0.5)
...
class EyeModel_BasicRGBSequence(Sequence):
    def __init__(self, data, dataset, batch_size, num_frames, shape_rgb, shape_depth):
        self.data = data
        self.dataset = dataset
        self.batch_size = batch_size
        self.num_frames = num_frames
