...
def evaluate(model, rgb, depth, crop, batch_size=6, verbose=False):
    ...
    for i in range(N//bs):    
        ...
        pred_y = scale_up(2, predict(model, x/255, minDepth=5, maxDepth=40, batch_size=bs)[:,:,:,0]) * 10.0
        
        # Test time augmentation: mirror image estimate
        pred_y_flip = scale_up(2, predict(model, x[...,::-1,:]/255, minDepth=5, maxDepth=40, batch_size=bs)[:,:,:,0]) * 10.0
